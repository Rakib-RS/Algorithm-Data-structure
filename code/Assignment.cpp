...
{
    if(n<0)     // hight & weight
        n=-n;   // it will give absolute value of integer
    return n;
}

float abs(float n) // Function overloading parameter one float
{
    if(n<0)
        n=-n;       // it will give absolute value of float
    return n;
}


class person  //it will inherit class(Teacher,Programmer,Student)
{
    public:
